...
Cross-shot continuity locks & quiet scenes — read `references/continuity-laws.md` (from The Trigger non-action corpus): per-prompt named LOCK blocks (FACE/LIGHT/DIALOGUE/HEIGHT/BRIGHTNESS/TEXT/KIT OVERRIDE — positive specifics + never-lists) picked from the continuity bible per shot; reference-image authority split (images decide identity, prompt decides light/color/motion); LIVING PERFORMANCE micro-acting for quiet scenes (first-frame staging, blink discipline, micro-motion lists); sound contracts (no-music as in-story rule, per-item ambience lists); prop physics contracts + prop-as-plot-device; readable-text scenes (TEXT LOCK whitelist + camera locked while text readable + PAGE organization); batch casting-sheet factory; Chinese-language prompt paradigm (78% of a scene's prompts fully Chinese — Chinese body + quoted original-language dialogue).
