...
`references/meeting-protocol.md` §12。

> ⏸️ **当前状态：暂停**（2026-09-12 由用户决定）。会议层**已在短剧 / 长片模式下默认开启**
> （见 `references/meeting-protocol.md` §0.1），但**收益仍未验证**——这是有意接受的取舍：
> 先上线拿真实使用数据，暂不为它单独跑 A/B。恢复实验时按下述设计走。
